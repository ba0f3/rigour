...
import (
	...
	"github.com/ctrlsam/rigour/pkg/scan"
...
var (
	...
	rootCmd = &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if config.stream {
				onEvent := func(ev scan.ScanEvent) {
				...
				err := scan.ScanTargetWithDiscoveryStream(cidrRange, createDiscoveryConfig(config), createScanConfig(config), onEvent)
				if err != nil {
			...
			results, err := scan.ScanTargetWithDiscovery(cidrRange, createDiscoveryConfig(config), createScanConfig(config))
			if err != nil {
				return fmt.Errorf("Failed running discovery+scan (%w)", err)
			}
